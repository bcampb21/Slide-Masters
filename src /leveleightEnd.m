...
#import "leveleight.h"
#import "leveleightcoundown.h"
#import "levelselect.h"
#import "levelnine.h"
#import <GameKit/GameKit.h>
#define IS_WIDESCREEN ( fabs( ( double )[ [ UIScreen mainScreen ] bounds ].size.height - ( double )568 )< DBL_EPSILON )

@implementation leveleightEnd
@synthesize bronze;
@synthesize silver;
...
-(IBAction)replay:(id)sender{
    ...
    leveleightcoundown *Sword = [[leveleightcoundown alloc] initWithNibName:nil bundle:nil];
    [self presentModalViewController:Sword animated:NO];
}
...
-(void)changeText{
    if (whatChallenge == 1) {
        ...
        theTime.text = [NSString stringWithFormat:@"17 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 504"];
    }
    if (whatChallenge == 2) {
        beatBlaze.text = [NSString stringWithFormat:@"Complete in under"];
        theTime.text = [NSString stringWithFormat:@"14.5 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 672"];
    }
    if (whatChallenge == 3) {
...
-(void)submitScore{ 
    GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"34jn3k45"];
...
-(IBAction)showLeaderboard:(id)sender{
    ...
    {  
        ...
        leaderboardController.category = @"34jn3k45";
...
- (void)viewDidLoad
{
    ...
    int term = 0;
        ...
        int medalb =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveleightbronze"];
        int medals =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveleightsilver"];
        int medalg =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveleightgold"];
    ...
    didGetMedal = 0;
    whichShow = 0; //  reset  
    int medal1 =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveleightbronze"];
    if (current < 21) {
        bronze.hidden= NO;
        if (medal1 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate leveleightbronzereward];
            [appDelegate leveleightbronze];
            whatChallenge = 1;
        }
    }
    
    int medal2 =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveleightsilver"];
    if (current < 17) {
        silver.hidden= NO;
        if (medal2 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate leveleightsilverreward];
            [appDelegate leveleightsilver];
            whatChallenge = 2;
        }
    }
    
    int medal3 =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveleightgold"];
    if (current < 14.5) {
        gold.hidden= NO;
        if (medal3 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate leveleightgoldreward];
            [appDelegate leveleightgold];
             whatChallenge = 3;
