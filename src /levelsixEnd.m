...
#import "levelsix.h"
#import "levelsixcountdown.h"
#import "levelselect.h"
#import "levelseven.h"
#import <GameKit/GameKit.h>
#define IS_WIDESCREEN ( fabs( ( double )[ [ UIScreen mainScreen ] bounds ].size.height - ( double )568 )< DBL_EPSILON )

@implementation levelsixEnd
@synthesize bronze;
@synthesize silver;
...
-(IBAction)replay:(id)sender{
    ...
    levelsixcountdown *Sword = [[levelsixcountdown alloc] initWithNibName:nil bundle:nil];
    [self presentModalViewController:Sword animated:NO];
}
...
-(void)changeText{
    if (whatChallenge == 1) {
        ...
        theTime.text = [NSString stringWithFormat:@"19 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 400"];
    }
    if (whatChallenge == 2) {
        beatBlaze.text = [NSString stringWithFormat:@"Complete in under"];
        theTime.text = [NSString stringWithFormat:@"16 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 520"];
    }
    if (whatChallenge == 3) {
...
-(void)submitScore{ 
    GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"fgb456g"];
...
-(IBAction)showLeaderboard:(id)sender{
    ...
    {  
        ...
        leaderboardController.category = @"fgb456g";
...
- (void)viewDidLoad
{
    ...
    int term = 0;
        ...
        int medalb =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsixbronze"];
        int medals =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsixsilver"];
        int medalg =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsixgold"];
    ...
    didGetMedal = 0;
    whichShow = 0; //  reset
    int medal1 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsixbronze"];
    if (current < 22) {
        bronze.hidden= NO;
        if (medal1 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelsixbronzereward];
            [appDelegate levelsixbronze];
            whatChallenge = 1;
        }
    }
    
    int medal2 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsixsilver"];
    if (current < 19) {
        silver.hidden= NO;
        if (medal2 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelsixsilverreward];
            [appDelegate levelsixsilver];
            whatChallenge = 2;
        }
    }
    
    int medal3 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsixgold"];
    if (current < 16) {
        gold.hidden= NO;
        if (medal3 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelsixgoldreward];
            [appDelegate levelsixgold];
            whatChallenge = 3;
