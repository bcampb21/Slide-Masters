...
#import "levelten.h"
#import "leveltencountdown.h"
#import "levelselect.h"
#import <GameKit/GameKit.h>
#define IS_WIDESCREEN ( fabs( ( double )[ [ UIScreen mainScreen ] bounds ].size.height - ( double )568 )< DBL_EPSILON )

@implementation leveltenEnd
@synthesize bronze;
@synthesize silver;
...
-(IBAction)replay:(id)sender{
    ...
    leveltencountdown *Sword = [[leveltencountdown alloc] initWithNibName:nil bundle:nil];
    [self presentModalViewController:Sword animated:NO];
}
...
-(void)changeText{
    if (whatChallenge == 1) {
        ...
        theTime.text = [NSString stringWithFormat:@"19.5 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 630"];
    }
    if (whatChallenge == 2) {
        beatBlaze.text = [NSString stringWithFormat:@"Complete in under"];
        theTime.text = [NSString stringWithFormat:@"16 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 840"];
    }
    if (whatChallenge == 3) {
...
-(void)submitScore{ 
    GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"hjy687hj"];
...
-(IBAction)showLeaderboard:(id)sender{
    ...
    {  
        ...
        leaderboardController.category = @"hjy687hj";
...
- (void)viewDidLoad
{
    ...
    int term = 0;
        ...
        int medalb =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveltenbronze"];
        int medals =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveltensilver"];
        int medalg =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveltengold"];
    ...
    didGetMedal = 0;
    whichShow = 0; //  reset
    int medal1 =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveltenbronze"];
    if (current < 22.5) {
        bronze.hidden= NO;
        if (medal1 ==0) {
            ...
            didGetMedal = 1;
            [appDelegate leveltenbronzereward];
            [appDelegate leveltenbronze];
             whatChallenge = 1;
        }
    }
    
    int medal2 =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveltensilver"];
    if (current < 19.5) {
        silver.hidden= NO;
        if (medal2 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate leveltensilverreward];
            [appDelegate leveltensilver];
             whatChallenge = 2;
        }
    }
    
    int medal3 =[[NSUserDefaults standardUserDefaults] integerForKey:@"leveltengold"];
    if (current < 16) {
        gold.hidden= NO;
        if (medal3 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate leveltengoldreward];
            [appDelegate leveltengold];
             whatChallenge = 3;
