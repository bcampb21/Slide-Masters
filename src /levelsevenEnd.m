...
#import "levelsevenEnd.h"
#import "levelseven.h"
#import "levelsevencountdown.h"
#import "levelselect.h"
#import "leveleight.h"
#import <GameKit/GameKit.h>
#define IS_WIDESCREEN ( fabs( ( double )[ [ UIScreen mainScreen ] bounds ].size.height - ( double )568 )< DBL_EPSILON )

@implementation levelsevenEnd
...
-(IBAction)replay:(id)sender{
    ...
    levelsevencountdown *Sword = [[levelsevencountdown alloc] initWithNibName:nil bundle:nil];
    [self presentModalViewController:Sword animated:NO];
}
...
-(void)changeText{
    if (whatChallenge == 1) {
        ...
        theTime.text = [NSString stringWithFormat:@"13.5 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 432"];
    }
    if (whatChallenge == 2) {
        beatBlaze.text = [NSString stringWithFormat:@"Complete in under"];
        theTime.text = [NSString stringWithFormat:@"10.5 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 576"];
    }
    if (whatChallenge == 3) {
...
-(void)submitScore{ 
    GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"kjn5645g"];
...
-(IBAction)showLeaderboard:(id)sender{
    ...
    {  
        ...
        leaderboardController.category = @"kjn5645g";
...
- (void)viewDidLoad
{
    ...
    andEarn.center = position2;
     ...
     current =[[NSUserDefaults standardUserDefaults] floatForKey:@"current7"];
    ...
    int term = 0;
        ...
        int medalb =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsevenbronze"];
        int medals =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsevensilver"];
        int medalg =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsevengold"];
    ...
    didGetMedal = 0;
    whichShow = 0; //  reset
    int medal1 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsevenbronze"];
    if (current < 18) {
        bronze.hidden= NO;
        if (medal1 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelsevenbronzereward];
            [appDelegate levelsevenbronze];
            whatChallenge = 1;
        }
    }
    
    int medal2 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsevensilver"];
    if (current < 13.5) {
        silver.hidden= NO;
        if (medal2 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            if (didGetMedal == 0 && whichShow == 0) {
             ...
             didGetMedal = 1;
            [appDelegate levelsevensilverreward];
            [appDelegate levelsevensilver];
             whatChallenge = 2;
        }
    }
    
    int medal3 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelsevengold"];
    if (current < 10.5) {
        gold.hidden= NO;
        if (medal3 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelsevengoldreward];
            [appDelegate levelsevengold];
            whatChallenge = 3;
