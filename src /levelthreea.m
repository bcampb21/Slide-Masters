...
-(void)check{
    ...
    if (type == 1 && slider.value > 98) {
        ...
        if (direction == 15) {
            ...
            [appDelegate stoplevelthreetimer];
            
            sup =[[NSUserDefaults standardUserDefaults] floatForKey:@"high3"];
            current =[[NSUserDefaults standardUserDefaults] floatForKey:@"current3"];
            
            if (sup ==0) {
                [appDelegate savehigh3];
                int gotBetterTime = 1;
        ...
        [UIView commitAnimations];
        if (direction == 15) {
            levelthreeEnd *Sword = [[levelthreeEnd alloc] initWithNibName:nil bundle:nil];
