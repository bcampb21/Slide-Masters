...
#import "levelnine.h"
#import "levelninecountdown.h"
#import "levelselect.h"
#import "levelten.h"
#import <GameKit/GameKit.h>
#define IS_WIDESCREEN ( fabs( ( double )[ [ UIScreen mainScreen ] bounds ].size.height - ( double )568 )< DBL_EPSILON )

@implementation levelnineEnd
@synthesize bronze;
@synthesize silver;
...
-(IBAction)replay:(id)sender{
    ...
    levelninecountdown *Sword = [[levelninecountdown alloc] initWithNibName:nil bundle:nil];
    [self presentModalViewController:Sword animated:NO];
}
...
-(void)changeText{
    if (whatChallenge == 1) {
        ...
        andEarn.text = [NSString stringWithFormat:@"and earn 540"];
    }
    if (whatChallenge == 2) {
        beatBlaze.text = [NSString stringWithFormat:@"Complete in under"];
        theTime.text = [NSString stringWithFormat:@"16 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 720"];
    }
    if (whatChallenge == 3) {
...
-(void)submitScore{ 
    GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"qwfh65"];
...
-(IBAction)showLeaderboard:(id)sender{
    ...
    {  
        ...
        leaderboardController.category = @"qwfh65";
...
- (void)viewDidLoad
{
    ...
    int term = 0;
        ...
        int medalb =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelninebronze"];
        int medals =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelninesilver"];
        int medalg =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelninegold"];
    ...
    didGetMedal = 0;
    whichShow = 0; //  reset
    int medal1 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelninebronze"];
    if (current < 23) {
        bronze.hidden= NO;
        if (medal1 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelninebronzereward];
            [appDelegate levelninebronze];
             whatChallenge = 1;
        }
    }
    
    int medal2 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelninesilver"];
    if (current < 19) {
        silver.hidden= NO;
        if (medal2 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelninesilverreward];
            [appDelegate levelninesilver];
             whatChallenge = 2;
        }
    }
    
    int medal3 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelninegold"];
    if (current < 16) {
        gold.hidden= NO;
        if (medal3 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelninegoldreward];
            [appDelegate levelninegold];
             whatChallenge = 3;
