...
-(IBAction)replay:(id)sender{
    ...
    levelfourcountdown *Sword = [[levelfourcountdown alloc] initWithNibName:nil bundle:nil];
    [self presentModalViewController:Sword animated:NO];
}
...
-(void)changeText{
    if (whatChallenge == 1) {
        ...
        theTime.text = [NSString stringWithFormat:@"10 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 330"];
    }
    if (whatChallenge == 2) {
        beatBlaze.text = [NSString stringWithFormat:@"Complete in under"];
        theTime.text = [NSString stringWithFormat:@"8.8 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 450"];
    }
    if (whatChallenge == 3) {
...
-(void)submitScore{ 
    GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"h56uh5"];
...
-(IBAction)showLeaderboard:(id)sender{
    ...
    {  
        ...
        leaderboardController.category = @"h56uh5";
...
- (void)viewDidLoad
{
    ...
    int term = 0;
        ...
        int medalb =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfourbronze"];
        int medals =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfoursilver"];
        int medalg =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfourgold"];
    ...
    didGetMedal = 0;
    whichShow = 0; //  reset       
    int medal1 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfourbronze"];
    if (current < 13) {
        bronze.hidden= NO;
        if (medal1 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelfourbronzereward];
            [appDelegate levelfourbronze];
            whatChallenge = 1;
        }
    }
    
    int medal2 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfoursilver"];
    if (current < 10) {
        silver.hidden= NO;
        if (medal2 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelfoursilverreward];
            [appDelegate levelfoursilver];
             whatChallenge = 2;
        }
    }
    
    int medal3 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfourgold"];
    if (current < 8.8) {
        gold.hidden= NO;
        if (medal3 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelfourgoldreward];
            [appDelegate levelfourgold];
             whatChallenge = 3;
