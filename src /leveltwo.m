...
- (void)viewDidLoad
{
    ...
    step = 1;
    float highScore =[[NSUserDefaults standardUserDefaults] floatForKey:@"high2"];
    ...
    int term = 0;
    int first =[[NSUserDefaults standardUserDefaults] floatForKey:@"first2"];
    if (first == 0) {
        ...
        slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
        [appDelegate firstTwo];
        int showUnlock = 1;
        [[NSUserDefaults standardUserDefaults] setInteger:showUnlock forKey:@"unlockFlex"];  
        [[NSUserDefaults standardUserDefaults] synchronize];
        NSLog(@"FIRST TIMER");
