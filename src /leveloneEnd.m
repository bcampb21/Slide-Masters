...
//  Slide Masters
//
//  Created by Blake Campbell on 4/25/12.
...
@implementation leveloneEnd
@synthesize bronze;
@synthesize silver;
@synthesize gold, beatBlaze, andEarn, theTime,challenge,complete,next,replay,menu,topazBorder,myTopaz,potBorder,topaz1, topaz2, topaz3, topaz4, topazText, topazTipBox, oTopaz;
...
-(IBAction)showLeaderboard:(id)sender{
    ...
    {  
        ...
        leaderboardController.category = @"8743bfdjs";
...
-(void)counter{
    ...
    if (step == 5) {
        ...
        [UILabel setAnimationRepeatAutoreverses:NO];
            ...
            if (IS_WIDESCREEN) {
                CGPoint position = CGPointMake(160.0f, 465.0f);
                topazText.center = position;
            }
            else{
                CGPoint position = CGPointMake(160.0f, 407.0f);
                topazText.center = position;
            }
        [UILabel commitAnimations];
        ...
        [UIView setAnimationRepeatAutoreverses:NO];
            ...
            if (IS_WIDESCREEN) {
                CGPoint position1 = CGPointMake(160.0f, 465.0f);
                topazTipBox.center = position1;
            }
            else{
                CGPoint position1 = CGPointMake(160.0f, 407.0f);
                topazTipBox.center = position1;
            }
            [UIView commitAnimations];
        }
        ...
        if (time == 40){
            ...
            CGPoint position1 = CGPointMake(160.0f, 650.0f);
            topazTipBox.center = position1;
            [UIView commitAnimations];
        }
        if (time == 43) {
...
-(void)submitScore{ 
    GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"8743bfdjs"];
...
- (void)viewDidLoad
{
    ...
    // Show the challenge that they are on
    int term = 0;
    int medalB =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelonebronze"];
    int medalS =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelonesilver"];
    int medalG =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelonegold"];
...
//////////////////////////
    ...
    if (current < 18) {
        ...
        if (medal1 ==0) {
            ...
            didGetMedal = 1;
            [appDelegate levelonebronzereward];
            [appDelegate levelonebronze];
            whatChallenge = 1;
        }
    }
    ...
    if (current < 12) {
        ...
        if (medal2 ==0) {
            ...
            didGetMedal = 1;
            [appDelegate levelonesilverreward];
            [appDelegate levelonesilver];
            whatChallenge = 2;
        }
    }
    ...
    if (current < 7) {
        ...
        if (medal3 ==0) {
            ...
            didGetMedal = 1;
            [appDelegate levelonegoldreward];
            [appDelegate levelonegold];
            whatChallenge = 3;
