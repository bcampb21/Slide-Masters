...
-(IBAction)replay:(id)sender{
    ...
    levelfivecountdown *Sword = [[levelfivecountdown alloc] initWithNibName:nil bundle:nil];
    [self presentModalViewController:Sword animated:NO];
}
...
-(void)changeText{
    if (whatChallenge == 1) {
        ...
        andEarn.text = [NSString stringWithFormat:@"and earn 374"];
    }
    if (whatChallenge == 2) {
        beatBlaze.text = [NSString stringWithFormat:@"Complete in under"];
        theTime.text = [NSString stringWithFormat:@"10 SECONDS"];
        andEarn.text = [NSString stringWithFormat:@"and earn 476"];
    }
    if (whatChallenge == 3) {
...
-(void)submitScore{ 
    GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"jkbnk564"];
...
-(IBAction)showLeaderboard:(id)sender{
    ...
    {  
        ...
        leaderboardController.category = @"jkbnk564";
...
- (void)viewDidLoad
{
    ...
    int term = 0;
        ...
        int medalb =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfivebronze"];
        int medals =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfivesilver"];
        int medalg =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfivegold"];
    ...
    int medal1 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfivebronze"];
    if (current < 16) {
        bronze.hidden= NO;
        if (medal1 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelfivebronzereward];
            [appDelegate levelfivebronze];
            whatChallenge = 1;
        }
    }
    
    int medal2 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfivesilver"];
    if (current < 12) {
        silver.hidden= NO;
        if (medal2 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelfivesilverreward];
            [appDelegate levelfivesilver];
             whatChallenge = 2;
        }
    }
    
    int medal3 =[[NSUserDefaults standardUserDefaults] integerForKey:@"levelfivegold"];
    if (current < 10) {
        gold.hidden= NO;
        if (medal3 ==0) {
            slidemastersAppDelegate *appDelegate = (slidemastersAppDelegate *)[[UIApplication sharedApplication] delegate];
            ...
            didGetMedal = 1;
            [appDelegate levelfivegoldreward];
            [appDelegate levelfivegold];
            whatChallenge = 3;
